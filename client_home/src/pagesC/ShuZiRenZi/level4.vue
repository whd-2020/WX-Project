...
export default {
  ...
  onLoad(options) {
    ...
    this.resetTimer();
    setTimeout(() => {
      this.playTyping('现在你要学习两位数的数字知识，分清十位和个位的组成规律，认识更多数字，完成进阶数字认知练习吧～')
    }, 1000);
  },
  onShow() {
    if (this.question) {
  ...
  methods: {
    ...
    isNumberNeedScale(num) {
      ...
      // 当选中正确答案时，所有数字块都不需要缩放
      if (userNum === correctNum) {
        return false;
      }

      // 如果选中的数字比正确答案小，则比当前选中数字大的所有数字块需要缩放
      if (userNum < correctNum) {
        return numValue > userNum;
      }

      // 如果选中的数字比正确答案大，则比当前选中数字小的所有数字块需要缩放
      if (userNum > correctNum) {
        return numValue < userNum;
      }
