...
export default {
  ...
  data() {
    return {
      ...
      showWelcomeText: true, // 控制是否显示欢迎文字
      preloadedQuestion: null, // 预加载的题目数据
      preloadedIcon: '', // 预加载的图标
    };
  },
  onLoad(options) {
    ...
    // 延迟一下再显示欢迎文字，确保预加载完成
    setTimeout(() => {
      this.playTyping('小朋友好呀，我们先来认识神奇的算筹！记住规则：竖着站立表示1，横着躺下表示5，点击对应木片，就可以组合摆出不同数字，跟着一起学习试一试吧～')
    }, 200);
  },
  computed: {
    // 获取目标数字文本
