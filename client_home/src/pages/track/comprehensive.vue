<template>
  <view class="track-page">
    <view class="track-content">
      <image 
        class="track-bg" 
        src="/static/images/games/number_introduction.png" 
        mode="aspectFill"
      />
      <view class="track-info">
        <view class="track-title">数字认知</view>
        <view class="desc-row">
          <view class="track-desc">认识数字，理解数字的含义和应用</view>
      ...
      <view class="intro-card" :class="{ 'card-visible': showIntroCard }" @click.stop>
        ...
        <view class="card-header">
          <view class="card-header-inner">
            <text class="card-title">📜 数字认知</text>
          </view>
        </view>
        <view class="card-content">
          <text class="intro-text">欢迎来到现代数字世界！我们日常用最简单的阿拉伯数字记录各类物品数量，方便又快捷。今天你将化身生活小管理员，根据任务给出的统计数量，匹配并认知对应数字，把从古至今学到的计数本领融会贯通，成为真正的计数小达人。</text>
        </view>
...
export default {
  ...
  onShow() {
    ...
    // 只在第一次进入时显示介绍卡片
    const hasSeenIntro = uni.getStorageSync('comprehensive_intro_shown');
    if (!hasSeenIntro) {
      setTimeout(() => {
  ...
  methods: {
    ...
    startLevel(index) {
      ...
      const levelNo = Number(level.levelOrder || level.levelId || (index + 1));
      const levelId = level.levelId || levelNo;
      const levelName = level.levelName || `关卡 ${levelNo}`;
